...
defmodule Fluxspace.EntityTest do
  ...
  defmodule Life do
    ...
    defmodule Behaviour do
      ...
      def handle_call({:damage, damage_points}, entity) do
        new_entity = update_attribute(entity, Life, fn(life) -> %Life{life | health: life.health - damage_points} end)
  ...
  test "Can add handler/behaviour to Entity" do
    {:ok, _entity_uuid, entity_pid} = Entity.start_plain
    entity_pid |> Life.register
    entity_pid |> Radio.register(__MODULE__)

    100 = Entity.call_behaviour(entity_pid, Life.Behaviour, :health)
    80 = Entity.call_behaviour(entity_pid, Life.Behaviour, {:damage, 20})
  end
end
